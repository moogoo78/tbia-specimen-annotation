...
const en = {
  ...
  walk: {
    ...
    s3Done: "When it finishes it tells you how many fields the AI wrote (one field counts as one).",

    s4Title: "4. Check it ⭐ the step that matters most",
    s4Body: "The proposals appear below under Annotation history, each marked ✦ AI.",
    s4Legend: "Struck-through text is the old value, the arrow points at the new one; 🚫 means it was empty before. Go through them one at a time against the label you just read:",
    ...
    s4Fix: "If something looks wrong, press Use to pull the value into the form below, correct it, then press Submit annotation — what goes in is your corrected version.",

    s5Title: "5. After it goes for review",
    s5Body: "The AI's proposals are already in the Submitted state; a reviewer takes it from here and there's nothing more for you to do.",
    ...
    faqA1: "No. The original record is read-only. Everything you send is a suggestion attached beside it, and it only counts once a reviewer accepts it.",
...
const zh: typeof en = {
  ...
  walk: {
    ...
    s3Done: "好了會告訴你 AI 寫了幾個欄位（一個欄位算一筆）：",

    s4Title: "4. 檢查 ⭐ 最重點的步驟",
    s4Body: "建議值出現在下面的「標註紀錄」，每筆都標示著 ✦ AI。",
    s4Legend: "刪除線是舊的、箭號後面是新的；🚫 表示本來是空的。拿它跟你剛剛看的標籤一項一項對：",
    ...
    s4Fix: "如果覺得哪一筆不對，按「填入」把值帶進下面的表單，改成對的，再按「送出標註」—— 送出去的就是你改過的版本。",

    s5Title: "5. 送審之後",
    s5Body: "AI 寫的建議值狀態已經是「已送出」，接下來等審核者處理，不用再做什麼。",
    s5Dash: "想看自己累積了多少、有幾筆被採納，到「貢獻儀表板」：",

    faqTitle: "常見問題",
    faqQ1: "AI 讀錯了會不會把原始資料弄壞？",
    faqA1: "不會。原始紀錄是唯讀的，你送的一切都只是附加在旁邊的建議，而且要審核過才算數。",
    faqQ2: "為什麼一定要先把照片放大？",
